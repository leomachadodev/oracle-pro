      ...
      <div style={{maxWidth:'700px',margin:'0 auto',padding:'48px 24px'}}>
        ...
        <div style={{background:'linear-gradient(135deg,#111,#1a1500)',border:`1px solid ${color}44`,borderRadius:'16px',padding:'40px',marginBottom:'28px',textAlign:'center',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{position:'relative',zIndex:2}}>
            ...
            <div style={{fontSize:'11px',letterSpacing:'4px',color,textTransform:'uppercase',marginBottom:'10px'}}>Certificado White Label</div>
            <div style={{fontSize:'28px',fontWeight:'900',color,marginBottom:'10px'}}>{config.business_name||'Seu Negócio'}</div>
            ...
            <div style={{display:'inline-block',background:`${color}15`,border:`1px solid ${color}44`,borderRadius:'8px',padding:'10px 24px',fontSize:'13px',color,letterSpacing:'2px',fontFamily:'monospace',marginBottom:'16px'}}>
        ...
        {modules.length > 0 && (
          <div style={{background:'#111',border:'1px solid #1e1e1e',borderRadius:'12px',padding:'24px',marginBottom:'20px'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
              {modules.map((mod,mi)=>(
                <div key={mi}>
                  <div style={{fontSize:'11px',fontWeight:'700',color,marginBottom:'8px',letterSpacing:'1px',textTransform:'uppercase'}}>{mod.title}</div>
                  <div style={{display:'flex',flexDirection:'column',gap:'6px'}}>
                    {mod.files?.map((file,fi)=>(
                      <a key={fi} href={file.url} target="_blank" rel="noreferrer"
                        style={{display:'flex',alignItems:'center',gap:'12px',background:'#0d0d0d',borderRadius:'8px',padding:'10px 14px',border:'1px solid #1e1e1e',textDecoration:'none'}}
                        ...
                        <div style={{fontSize:'11px',color,fontWeight:'700'}}>⬇ Baixar</div>
                      </a>
                    ))}
                  </div>
                </div>
...
**Ctrl+S** e depois no terminal:
```
git add . && git commit -m "Pagina entrega WL" && git push