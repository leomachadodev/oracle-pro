cat > app/curso/page.js << 'ENDOFFILE'
'use client'
...
export default function Curso() {
  ...
  const curso = {
    ...
    modulos: [
      { nome: 'Fundamentos', aulas: [
        { titulo: 'Introdução ao tráfego', dur: '8min', done: true, video: '' },
        { titulo: 'Métricas essenciais', dur: '12min', done: true, video: '' },
        { titulo: 'Estrutura de campanhas', dur: '15min', done: true, video: '' },
        { titulo: 'Pixels e rastreamento', dur: '18min', done: false, video: '' },
        { titulo: 'Públicos e segmentação', dur: '20min', done: false, video: '' },
        { titulo: 'Orçamento e lances', dur: '14min', done: false, video: '' },
      ]},
      { nome: 'Meta Ads', aulas: [
        { titulo: 'Criando conta Business', dur: '10min', done: false, video: '' },
        { titulo: 'Campanhas de conversão', dur: '22min', done: false, video: '' },
        { titulo: 'Criativos que convertem', dur: '18min', done: false, video: '' },
      ]},
      { nome: 'Google Ads', aulas: [
        { titulo: 'Google Search', dur: '20min', done: false, video: '' },
        { titulo: 'Google Display', dur: '15min', done: false, video: '' },
      ]},
      { nome: 'TikTok Ads', aulas: [
        { titulo: 'Introdução TikTok Ads', dur: '14min', done: false, video: '' },
        { titulo: 'Criando campanhas', dur: '18min', done: false, video: '' },
      ]},
    ],
    materiais: [
      { nome: 'Checklist instalação do Pixel', tipo: 'PDF', tamanho: '380 KB', url: '#' },
      { nome: 'Planilha de eventos', tipo: 'XLSX', tamanho: '220 KB', url: '#' },
      { nome: 'Guia de eventos personalizados', tipo: 'Link', tamanho: 'Externo', url: '#' },
    ]
  }
  ...
  const aulaAtual = curso.modulos.flatMap(m => m.aulas)[aulaAtiva]
  const totalAulas = curso.modulos.flatMap(m => m.aulas).length
  let aulaIndex = 0
    ...
    <div style={{height:'100vh',background:s.bg,fontFamily:'sans-serif',color:s.text,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <nav style={{background:s.nav,borderBottom:`1px solid ${s.border}`,padding:'0 20px',height:'50px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
        <div style={{display:'flex',alignItems:'center',gap:'14px'}}>
          ...
          <div style={{fontSize:'11px',color:'#666',maxWidth:'300px',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{curso.nome}</div>
        </div>
      ...
      <div style={{display:'flex',flex:1,overflow:'hidden'}}>
        ...
        <div style={{flex:1,display:'flex',flexDirection:'column',overflowY:'auto',scrollbarWidth:'thin'}}>
          ...
          <div style={{background:'#000',width:'100%',aspectRatio:'16/9',maxHeight:'56vh',position:'relative',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
            {aulaAtual?.video ? (
              <iframe src={aulaAtual.video} style={{width:'100%',height:'100%',border:'none'}} allowFullScreen></iframe>
            ) : (
            ...
            <div style={{position:'absolute',bottom:0,left:0,right:0,padding:'14px 18px 12px',background:'linear-gradient(0deg,rgba(0,0,0,.95),transparent)'}}>
              ...
              <div style={{fontSize:'10px',color:'#888',marginTop:'2px'}}>Módulo {curso.modulos.findIndex(m => m.aulas.includes(aulaAtual)) + 1} · {aulaAtual?.dur}</div>
              <div style={{height:'3px',background:'#333',borderRadius:'2px',marginTop:'10px',cursor:'pointer'}}>
          ...
          <div style={{background:'#0d0d0d',borderBottom:`1px solid ${s.border}`,padding:'9px 18px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0}}>
            <div style={{display:'flex',gap:'8px'}}>
              <button onClick={() => setAulaAtiva(Math.max(0, aulaAtiva - 1))}
                style={{background:'#1a1a1a',border:`1px solid ${s.border}`,borderRadius:'6px',padding:'6px 13px',fontSize:'10px',color:s.text,cursor:'pointer',fontFamily:'sans-serif'}}>⬅ Anterior</button>
              <button style={{background:`linear-gradient(90deg,${s.accent},${s.accent2})`,color:'#000',border:'none',borderRadius:'6px',padding:'6px 13px',fontSize:'10px',fontWeight:'700',cursor:'pointer',letterSpacing:'.5px'}}>✓ Marcar concluída</button>
              <button onClick={() => setAulaAtiva(Math.min(totalAulas - 1, aulaAtiva + 1))}
                style={{background:'#1a1a1a',border:`1px solid ${s.border}`,borderRadius:'6px',padding:'6px 13px',fontSize:'10px',color:s.text,cursor:'pointer',fontFamily:'sans-serif'}}>Próxima ➡</button>
            </div>
            <select style={{background:'#1a1a1a',border:`1px solid ${s.border}`,borderRadius:'6px',padding:'5px 9px',fontSize:'10px',color:s.text,fontFamily:'sans-serif',outline:'none',cursor:'pointer'}}>
          ...
          <div style={{display:'flex',borderBottom:`1px solid ${s.border}`,padding:'0 18px',flexShrink:0}}>
            {[['sobre','Sobre a aula'],['materiais','Materiais'],['comentarios','Comentários']].map(([id,label]) => (
              <div key={id} onClick={() => setTab(id)}
                style={{padding:'11px 14px',fontSize:'11px',cursor:'pointer',borderBottom:`2px solid ${tab===id?s.accent:'transparent'}`,color:tab===id?s.accent:s.muted,transition:'all .15s'}}>
                {label}
              </div>
            ))}
          </div>
          ...
          <div style={{padding:'20px',flex:1}}>
            {tab === 'sobre' && (
              <div>
                <div style={{fontFamily:'sans-serif',fontSize:'18px',fontWeight:'700',letterSpacing:'1px',marginBottom:'8px'}}>{aulaAtual?.titulo}</div>
                <p style={{fontSize:'12px',color:'#888',lineHeight:1.7,marginBottom:'18px'}}>Nesta aula você vai aprender os conceitos fundamentais e aplicações práticas do tema abordado. Acompanhe com atenção e utilize os materiais de apoio disponíveis.</p>
              </div>
            )}
            {tab === 'materiais' && (
              <div>
                <div style={{fontSize:'10px',color:s.muted,marginBottom:'12px',letterSpacing:'1px',textTransform:'uppercase'}}>📎 Materiais da aula</div>
                <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                  {curso.materiais.map((mat, i) => (
                    <div key={i} onClick={() => window.open(mat.url,'_blank')}
                      style={{background:s.card,border:`1px solid ${s.border}`,borderRadius:'8px',padding:'10px 14px',display:'flex',alignItems:'center',gap:'12px',cursor:'pointer',transition:'border-color .15s'}}
            ...
            {tab === 'comentarios' && (
              <div style={{textAlign:'center',padding:'40px',color:s.muted,fontSize:'12px'}}>
        ...
        <div style={{width:'310px',background:'#0d0d0d',borderLeft:`1px solid ${s.border}`,display:'flex',flexDirection:'column',flexShrink:0,overflow:'hidden'}}>
          <div style={{padding:'14px 16px',borderBottom:`1px solid ${s.border}`}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'8px',marginTop:'8px'}}>
              ...
              <span style={{fontSize:'10px',color:s.muted}}>{curso.progresso}% · {aulaAtiva}/{totalAulas}</span>
            </div>
          </div>
          <div style={{padding:'8px 12px',borderBottom:`1px solid ${s.border}`}}>
            <input type="text" placeholder="🔍  Buscar aula..."
              style={{width:'100%',background:'#161616',border:`1px solid ${s.border}`,borderRadius:'5px',padding:'6px 10px',fontSize:'11px',color:s.text,outline:'none',fontFamily:'sans-serif'}}/>
          </div>
              ...
              return (
                <div key={mi} style={{borderBottom:`1px solid ${s.border}`}}>
                  <div onClick={() => setModAberto(modAberto === mi ? -1 : mi)}
                    style={{padding:'11px 14px',display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',transition:'background .15s',gap:'10px'}}
                    onMouseEnter={e=>e.currentTarget.style.background='#111'}
                    onMouseLeave={e=>e.currentTarget.style.background='transparent'}>
                    <div style={{display:'flex',alignItems:'center',gap:'9px',flex:1,minWidth:0}}>
                      <span style={{fontSize:'8px',color:s.muted,fontWeight:'700',letterSpacing:'1px',background:'#1a1a1a',padding:'2px 6px',borderRadius:'3px',flexShrink:0}}>MOD {mi+1}</span>
                      ...
                      {mod.aulas.map((aula, ai) => {
                        ...
                        return (
                          <div key={ai} onClick={() => setAulaAtiva(globalIndex)}
                            style={{display:'flex',alignItems:'center',gap:'9px',padding:'8px 14px 8px 28px',cursor:'pointer',transition:'background .15s',borderLeft:`2px solid ${isActive?s.accent:'transparent'}`,background:isActive?'rgba(240,165,0,.07)':'transparent'}}
